  ...
  return (
    <Link href={`/tops/${slug}`} className="group block space-y-4">
      ...
      <div className="px-2 space-y-2">
        <h3 className="text-xl font-bold tracking-tight group-hover:underline decoration-1 underline-offset-4 italic">{title}</h3>
        
        <div className="flex gap-1.5">
          {variants?.sizes?.map(size => (
            <span key={size} className="text-[9px] font-bold border border-border/60 px-1.5 py-0.5 rounded text-muted-foreground">
              {size}
            </span>
          ))}
        </div>
    ...
    <Link href={`/tops/${slug}`} className="group block space-y-3">
      ...
      <div className="space-y-1">
        ...
        <div className="flex gap-1">
          {variants?.sizes?.map(size => (
            <span key={size} className="text-[8px] font-bold text-muted-foreground/60 uppercase">
              {size}
            </span>
          ))}
        </div>
